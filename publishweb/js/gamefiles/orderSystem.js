 ...
 * @author pmo
 */
import { ItemType, getItemMetadata } from "./itemTypes.js";
/**
 ...
 */
export function generateOrder(allItemTypes = Object.values(ItemType).filter(v => typeof v === 'number')) {
 ...
 */
export function createOrderState(requiredItems) {
    return {
        requiredItems,
 ...
 */
export function spawnShelfItems(order, shelves, allItemTypes = Object.values(ItemType).filter(v => typeof v === 'number')) {
    if (shelves.length < order.length) {
        throw new Error(`Need at least ${order.length} shelves to spawn all required items`);
    }
    const updatedShelves = shelves.map(shelf => (Object.assign({}, shelf)));
 ...
 */
export function canInteract(playerPos, shelf) {
 ...
 */
export function tryPickup(shelf, orderState) {
 ...
 */
export function checkComplete(orderState) {
 ...
 */
export function getProgress(orderState) {
 ...
 */
export function getProgressText(orderState) {
    return `${orderState.collectedItems.size}/${orderState.requiredItems.length}`;
}
