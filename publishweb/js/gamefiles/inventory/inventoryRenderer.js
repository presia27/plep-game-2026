...
import { getItemMetadata } from "../ordermanagement/itemTypes.js";
import { ITEM_WIDTH, ITEM_HEIGHT } from "../ordermanagement/itemEntity.js";
...
export class InventoryRenderer {
     ...
     */
    constructor(posX, posY, inventoryManager) {
        this.posX = posX;
        this.posY = posY;
    ...
    draw(context) {
        ...
        inventory.forEach((value, key) => {
            const item = key;
            const itemMeta = getItemMetadata(item);
            ctx.drawImage(itemSprite, itemMeta.spriteFrameX, itemMeta.spriteFrameY, ITEM_WIDTH, ITEM_HEIGHT, this.posX + ((i * (ITEM_SIDE_WIDTH + BUFFER)) + BUFFER), this.posY + 36, ITEM_SIDE_WIDTH, ITEM_SIDE_WIDTH);
