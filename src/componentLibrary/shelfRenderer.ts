...
import { GameContext, IRenderer, IPosition } from "../classinterfaces.ts";
 ...
 */
export class ShelfRenderer implements IRenderer {
  private shelfImage: HTMLImageElement;
  ...
  private itemOffsetX: number;
  private itemOffsetY: number;
  ...
  constructor(
    ...
    itemOffsetX: number = 8,
    itemOffsetY: number = -10,
    scale: number = 2.0
  ) {
  ...
  public draw(context: GameContext): void {
    ...
    if (context.debug) {
      ...
      ctx.arc(
        itemCenterX,
        itemCenterY,
        this.shelfComponent.getInteractionRadius(),
        0,
        Math.PI * 2
      );
    ...
    ctx.drawImage(
      ...
      pos.x, 
      pos.y,
      this.shelfImage.width * this.scale,
      this.shelfImage.height * this.scale
    );
  }
   ...
   */
  public drawItem(context: GameContext): void {
