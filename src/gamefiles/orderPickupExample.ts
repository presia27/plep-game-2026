 ...
 */
export async function setupOrderPickupExample(
  gameEngine: GameEngine,
  assetManager: AssetManager,
  playerEntity: Entity
): Promise<OrderManager> {
 ...
 */
export const orderPickupInputMap: InputMapValue[] = [
  { type: "key", value: "e", action: InputAction.PICK_UP },
  { type: "key", value: "E", action: InputAction.PICK_UP }
];

/**
 * Simplified setup function that returns all components
 * for manual integration into your game
 */
export interface OrderSystemComponents {
  orderManager: OrderManager;
  shelfEntities: Entity[];
  shelfComponents: ShelfComponent[];
  uiEntity: Entity;
  uiRenderer: OrderUIRenderer;
}

export async function createOrderSystem(
  assetManager: AssetManager,
  shelfPositions: { x: number; y: number }[]
): Promise<OrderSystemComponents> {
  ...
  for (const pos of shelfPositions) {
    ...
    const renderer = new ShelfRenderer(
      shelfImage, itemsSpritesheet, positionComp, shelfComp,
      16, 16, 8, -10, 2.0
    );
  ...
  const uiEntity = new Entity();
  const uiRenderer = new OrderUIRenderer(
    initialOrder, itemsSpritesheet, 20, 20, 16, 16, 2.0, 40
  );
  uiEntity.setRenderer(uiRenderer);
